...
export class AIPlayPage implements Page {
        ...
        let html = `
            <div class="max-w-5xl mx-auto bg-gray-800 p-8 rounded-lg shadow-lg">
                ...
                <div class="mt-10 text-center">
                    ...
                    <div class="mt-6">
                        <a href="#game" class="text-neon-blue underline hover:text-blue-300">${t("backToGame")}</a>
                    </div>
                </div>
            </div>
        `;
    ...
    botCard(botId: string, imgSrc: string, name: string, desc: string, ringColor: string) {
        return `
            <div class="bot-card bg-gray-900 p-6 rounded-xl hover:ring-2 hover:ring-${ringColor} flex flex-col items-center shadow-md transition-all duration-200 cursor-pointer" data-bot="${botId}" data-ring="${ringColor}">
    ...
    setupBotSelection() {
        ...
        cards.forEach(card => {
            card.addEventListener("click", () => {
                ...
                selectedBotId = card.getAttribute("data-bot");
        ...
        playButton.addEventListener("click", () => {
            if (selectedBotId) {
                console.log(`🎮 Starting game vs ${selectedBotId}`);
                window.location.hash = `game?bot=${selectedBotId}`;
            }
        });
    }
}
