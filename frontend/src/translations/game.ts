export const gameTranslations = {
  title: {
    en: "Games",
    fr: "Jeux",
    de: "Spiele",
    ar: "الألعاب",
    ko: "게임"
  },
  aiTitle: {
    en: "🤖 Play against a Robo",
    fr: "🤖 Affronte un robot",
    de: "🤖 Gegen einen Roboter spielen",
    ar: "🤖 واجه الروبوت",
    ko: "🤖 로봇과 대결"
  },
  aiDesc: {
    en: "Face the ultimate AI challenge. Can you defeat the machine?",
    fr: "Relève le défi ultime de l'IA. Peux-tu battre la machine ?",
    de: "Stell dich der ultimativen KI-Herausforderung. Kannst du sie besiegen?",
    ar: "واجه التحدي النهائي للذكاء الاصطناعي. هل يمكنك هزيمة الآلة؟",
    ko: "최강 AI에 도전하세요. 기계를 이길 수 있을까요?"
  },
  localTitle: {
    en: "🎮 Play against a Friend Locally",
    fr: "🎮 Joue en local contre un ami",
    de: "🎮 Lokal gegen einen Freund spielen",
    ar: "🎮 العب ضد صديق محليًا",
    ko: "🎮 친구와 로컬 플레이"
  },
  localDesc: {
    en: "Two controllers, one screen. Who will take the crown?",
    fr: "Deux manettes, un écran. Qui remportera la couronne ?",
    de: "Zwei Controller, ein Bildschirm. Wer holt sich die Krone?",
    ar: "ذراعان للتحكم، شاشة واحدة، إذا من سيفوز؟",
    ko: "컨트롤러 2개, 한 화면. 왕관은 누구의 것?"
  },
  onlineTitle: {
    en: "🌐 Play against a Friend Online",
    fr: "🌐 Joue contre un ami en ligne",
    de: "🌐 Online gegen einen Freund spielen",
    ar: "🌐 العب ضد صديق عبر الإنترنت",
    ko: "🌐 친구와 온라인 대결"
  },
  onlineDesc: {
    en: "Distance is nothing—battle your friend anywhere, anytime!",
    fr: "La distance ne compte pas : affronte ton ami partout, à tout moment !",
    de: "Distanz ist egal – fordere deinen Freund überall und jederzeit heraus!",
    ar: "!المسافة لا تهم — تحدَّ صديقك في أي وقت ومن أي مكان",
    ko: "거리 상관없이 언제 어디서나 친구와 대결하세요!"
  },
  rulesTitle: {
    en: "Game Rules — PHΛN†ØM[404]",
    fr: "Règles du jeu — PHΛN†ØM[404]",
    de: "Spielregeln — PHΛN†ØM[404]",
    ar: "قواعد اللعبة — PHΛN†ØM[404]",
    ko: "게임 규칙 — PHΛN†ØM[404]"
  },
  goalTitle: {
    en: "Goal:",
    fr: "Objectif :",
    de: "Ziel:",
    ar: "الهدف:",
    ko: "목표:"
  },
  goal: {
    en: "Reach 11 points, win by 2.",
    fr: "Atteindre 11 points, gagner par 2.",
    de: "Erreiche 11 Punkte, gewinne mit 2 Punkten Vorsprung.",
    ar: "الوصول إلى 11 نقطة والفوز بفارق نقطتين.",
    ko: "11점에 도달하고 2점 차로 승리하세요."
  },
  match: {
    en: "Match: 1 set only.",
    fr: "Match : 1 set seulement.",
    de: "Spiel: Nur 1 Satz.",
    ar: "مباراة: مجموعة واحدة فقط.",
    ko: "경기: 세트 1개만."
  },
  serve: {
    en: "Serve:",
    fr: "Service :",
    de: "Aufschlag:",
    ar: "الإرسال:",
    ko: "서브:"
  },
  serveRule: {
    en: "Switch every 2 points. At 10–10: switch every point.",
    fr: "Changer tous les 2 points. À 10–10 : changer à chaque point.",
    de: "Wechsle alle 2 Punkte. Bei 10–10: wechsle bei jedem Punkt.",
    ar: "التبديل كل نقطتين. عند 10-10 : التبديل عند كل نقطة.",
    ko: "2점마다 서브 교체. 10-10에서는 매 포인트마다 교체."
  },
  indicator: {
    en: "Indicator:",
    fr: "Indicateur :",
    de: "Anzeige:",
    ar: "المؤشر:",
    ko: "표시기:"
  },
  indicatorRule: {
    en: "A light shows who serves.",
    fr: "Une lumière indique qui sert.",
    de: "Ein Licht zeigt, wer aufschlägt.",
    ar: "ضوء يُظهر من يقوم بالإرسال.",
    ko: "불빛이 서브하는 선수를 보여줍니다."
  },
  turn: {
    en: "Turn:",
    fr: "Tour :",
    de: "Zug:",
    ar: "الدور:",
    ko: "턴:"
  },
  turnRule: {
    en: "Players hit the ball alternately.",
    fr: "Les joueurs frappent la balle à tour de rôle.",
    de: "Die Spieler schlagen abwechselnd den Ball.",
    ar: "يضرب اللاعبون الكرة بالتناوب.",
    ko: "플레이어가 번갈아 공을 칩니다."
  },
  robotDuel: {
    en: "Robot Duel:",
    fr: "Duel contre robot :",
    de: "Roboter-Duell:",
    ar: "مبارزة ضد الروبوت:",
    ko: "로봇 대결:"
  },
  robotDuelRule: {
    en: "If playing against a robot, the player always has serve control.",
    fr: "Si vous jouez contre un robot, le joueur garde toujours le service.",
    de: "Gegen einen Roboter hat der Spieler immer das Aufschlagrecht.",
    ar: "إذا كنت تلعب ضد روبوت، يحتفظ اللاعب دائمًا بخدمة الإرسال.",
    ko: "로봇과 경기할 때 플레이어가 항상 서브 권한을 가집니다."
  },
  losePoint: {
    en: "Lose point:",
    fr: "Perte de point :",
    de: "Punktverlust:",
    ar: "خسارة نقطة:",
    ko: "포인트 상실:"
  },
  rulesButton: {
    en: "Rules",
    fr: "Règles",
    de: "Regeln",
    ar: "القواعد",
    ko: "규칙"
  },
  duelWinner: {
    en: "Winner of the Duel!",
    fr: "Vainqueur du Duel !",
    de: "Sieger des Duells!",
    ar: "! الفائز بالمبارزة",
    ko: "결투의 승자!"
  },
  tournamentChampion: {
    en: "Champion of the Tournament!",
    fr: "Champion du tournoi !",
    de: "Turniersieger!",
    ar: "! بطل البطولة",
    ko: "토너먼트 챔피언!"
  },
  rematch: {
    en: "Rematch",
    fr: "Revanche",
    de: "Rückspiel",
    ar: "! مباراة جديدة",
    ko: "재경기"
  },
  newTournament: {
    en: "New Tournament",
    fr: "Nouveau tournoi",
    de: "Neues Turnier",
    ar: "! بطولة جديدة",
    ko: "새 토너먼트"
  },
  newDuel: {
    en: "New Duel",
    fr: "Nouveau duel",
    de: "Neues Duell",
    ar: "! مبارزة جديدة",
    ko: "새로운 대결",
  },
  losePointRule: {
    en: "If ball exits without contact.",
    fr: "Si la balle sort sans contact.",
    de: "Wenn der Ball ohne Berührung das Spielfeld verlässt.",
    ar: "إذا خرجت الكرة بدون تلامس.",
    ko: "공이 닿지 않고 밖으로 나가면 점수를 잃습니다."
  },
  customizeGame: {
    en: "Customize",
    fr: "Personnaliser",
    de: "Anpassen",
    ar: "تخصيص",
    ko: "사용자 지정"
},
} as const;

export const localPlayTranslations = {
  titleLocalPlay: {
    en: "Local Play",
    fr: "Jeu Local",
    de: "Lokales Spiel",
    ar: "اللعب المحلي",
    ko: "로컬 플레이"
  },
  startPlay: {
    en: "Start Playing",
    fr: "Commencer à jouer",
    de: "Spiel starten",
    ar: "ابدأ اللعب",
    ko: "게임 시작"
  },
  playersMissingError: {
    en: "Please fill in both player names and select an avatar.",
    fr: "Veuillez remplir les deux noms de joueurs et sélectionner un avatar.",
    de: "Bitte gib beide Spielernamen ein und wähle einen Avatar.",
    ar: "يرجى إدخال اسمي اللاعبين واختيار صورة رمزية.",
    ko: "두 플레이어 이름을 입력하고 아바타를 선택하세요."
  },
  connectedPlayer: {
    en: "You (Connected Player)",
    fr: "Vous (Joueur connecté)",
    de: "Du (verbundener Spieler)",
    ar: "أنت (اللاعب المتصل)",
    ko: "당신 (연결된 플레이어)"
  },
  playerNamePlaceholder: {
    en: "Enter player name",
    fr: "Entrez le nom du joueur",
    de: "Spielernamen eingeben",
    ar: "أدخل اسم اللاعب",
    ko: "플레이어 이름 입력"
  },
  readyChampion: {
    en: "Ready to Rule?",
    fr: "Prêt à régner ?",
    de: "Bereit zu herrschen?",
    ar: "مستعد للسيطرة؟",
    ko: "통치할 준비가 되었나요?"
  },
  chooseYourHero: {
    en: "Choose Your Hero!",
    fr: "Choisis ton héros !",
    de: "Wähle deinen Helden!",
    ar: "اختر بطلك!",
    ko: "당신의 영웅을 선택하세요!"
  },
  guestConnectedInfo: {
    en: "Yellow player connected... Guest, get ready to see stars?! ✨✨",
    fr: "Joueur jaune connecté... Invité, prépare-toi à voir des étoiles ! ✨✨",
    de: "Gelber Spieler verbunden... Gast, mach dich bereit, Sterne zu sehen! ✨✨",
    ar: "تم توصيل اللاعب الأصفر... استعد لرؤية النجوم أيها الزائر! ✨✨",
    ko: "노란 플레이어가 연결되었습니다... 게스트, 별을 볼 준비를 하세요! ✨✨"
  },
  back: {
    en: "⬅ Back",
    fr: "⬅ Retour",
    de: "⬅ Zurück",
    ar: "⬅ رجوع",
    ko: "⬅ 뒤로"
  }
} as const;

export const onlinePlayTranslations = {
  title: {
    en: "Online Duel Mode",
    fr: "Mode Duel En Ligne",
    de: "Online-Duellmodus",
    ar: "وضع المبارزة عبر الإنترنت",
    ko: "온라인 듀얼 모드"
  },
  description: {
    en: "Select an opponent from the list to start a remote duel!",
    fr: "Sélectionnez un adversaire dans la liste pour commencer un duel à distance !",
    de: "Wähle einen Gegner aus der Liste, um ein Fern-Duell zu starten!",
    ar: "اختر خصمًا من القائمة لبدء مبارزة عن بُعد!",
    ko: "목록에서 상대를 선택하여 원격 대결을 시작하세요!"
  },
  play: {
    en: "Start Duel",
    fr: "Commencer le duel",
    de: "Duell starten",
    ar: "ابدأ المبارزة",
    ko: "대결 시작"
  },
  back: {
    en: "⬅ Back",
    fr: "⬅ Retour",
    de: "⬅ Zurück",
    ar: "⬅ رجوع",
    ko: "⬅ 뒤로"
  },
  duelInvite: {
    en: "Hey!👋 Duel time! Join the table 🏓 (Click to accept!!)",
    fr: "Hey !👋 C'est l'heure du duel ! Rejoins la table 🏓 (Clique pour accepter !!)",
    de: "Hey!👋 Duell-Zeit! Komm an den Tisch 🏓 (Klicke, um anzunehmen!!)",
    ar: "مرحباً!👋 حان وقت التحدي! انضم إلى الطاولة 🏓 (انقر للقبول!!)",
    ko: "안녕!👋 듀얼 시간이에요! 탁자에 참여하세요 🏓 (클릭해서 수락하세요!!)"
}
} as const;


export const aiPlayTranslations = {
  chooseRobotTitle: {
    en: "Choose Your Robo Opponent",
    fr: "Choisissez votre adversaire robot",
    de: "Wähle deinen Roboter-Gegner",
    ar: "اختر خصمك الروبوتي",
    ko: "로봇 상대를 선택하세요"
  },
  playBtn: {
    en: "Play",
    fr: "Jouer",
    de: "Spielen",
    ar: "العب",
    ko: "플레이"
  },
  backToGame: {
    en: "Back to Game Modes",
    fr: "Retour aux modes de jeu",
    de: "Zurück zu den Spielmodi",
    ar: "العودة إلى أوضاع اللعب",
    ko: "게임 모드로 돌아가기"
  },
  bot1Name: {
    en: "RoboZ",
    fr: "RoboZ",
    de: "RoboZ",
    ar: "روبو زد",
    ko: "로보Z"
  },
  bot1Desc: {
    en: "A balanced bot with medium speed and smart defense.",
    fr: "Un robot équilibré, avec une vitesse moyenne et une défense intelligente.",
    de: "Ein ausgewogener Bot mit mittlerer Geschwindigkeit und kluger Verteidigung.",
    ar: "روبوت متوازن بسرعة متوسطة ودفاع ذكي.",
    ko: "중간 속도와 스마트한 방어를 가진 균형 잡힌 로봇입니다."
  },
  bot2Name: {
    en: "Mecha-X",
    fr: "Mecha-X",
    de: "Mecha-X",
    ar: "ميكا إكس",
    ko: "메카-X"
  },
  bot2Desc: {
    en: "Fast and aggressive, perfect for quick matches.",
    fr: "Rapide et agressif, parfait pour des parties rapides.",
    de: "Schnell und aggressiv – perfekt für kurze Matches.",
    ar: "سريع وعدواني، مثالي للمباريات السريعة.",
    ko: "빠르고 공격적인 스타일로 빠른 게임에 적합합니다."
  },
  bot3Name: {
    en: "CyberNova",
    fr: "CyberNova",
    de: "CyberNova",
    ar: "سايبر نوفا",
    ko: "사이버노바"
  },
  bot3Desc: {
    en: "Strategic bot with high accuracy and tough AI.",
    fr: "Robot stratégique avec une grande précision et une IA coriace.",
    de: "Strategischer Bot mit hoher Genauigkeit und starker KI.",
    ar: "روبوت استراتيجي بدقة عالية وذكاء اصطناعي قوي.",
    ko: "높은 정확도와 강력한 인공지능을 가진 전략적 로봇입니다."
  }
} as const;